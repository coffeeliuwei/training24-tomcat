...
async function doReset(){
  ...
  const ans=await LW.rest('/api/user', {action:'reset_password', username, code, newPassword});
...
async function loadPage(page=1){
  const ans=await LW.rest('/api/course', {action:'list', page, size:10});
  if(ans.error!=0) return showError(ans.reason);
...
async function onFilter(){
  ...
  const ans=await LW.rest('/api/course', {action:'filter', ...q});
  if(ans.error!=0) return showError(ans.reason);
...
async function doEnroll(courseId){
  const ans=await LW.rest('/api/enroll', {action:'enroll', courseId});
...
async function exportGrades(){
  const ans=await LW.rest('/api/student', {action:'grades_export'});
  if(ans.error!=0) return showError(ans.reason);
...
async function queryLogs(q){
  const ans=await LW.rest('/api/admin', {action:'logs_query', ...q});
  if(ans.error!=0) return showError(ans.reason);
...
async function loadRecommend(){
  const ans=await LW.rest('/api/student', {action:'recommend'});
  if(ans.error!=0) return showError(ans.reason);
