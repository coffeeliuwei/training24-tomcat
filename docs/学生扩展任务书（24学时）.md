...
4. CSV 导出内容优化
   - 后端改动：`StudentServlet?action=export_csv` 首行写表头 `课程,学分,成绩` 与生成时间；文件名 `grades-YYYYMMDD-HHMM.csv`
...
6. 课程推荐增强
   - 后端改动：`Db.recommend` 中加入“新课优先”（过去 24 小时新增课程权重提升）
...
case "reset_password":{
    ...
    boolean ok=com.training.db.Db.resetPassword(username,newPwd);
...
case "filter":{
    ...
    java.util.List<com.training.db.Db.Course> list=com.training.db.Db.filterCourses(min,max,day);
...
case "enroll":{
    ...
    com.training.db.Db.Course target=null; for(com.training.db.Db.Course c: com.training.db.Db.listCourses()) if(c.id.equals(courseId)) {target=c;break;}
    java.util.List<com.training.db.Db.Enrollment> mine=com.training.db.Db.listUserEnrollments(uid);
    java.util.Map<String,com.training.db.Db.Course> id2c=new java.util.HashMap<>(); for(com.training.db.Db.Course c: com.training.db.Db.listCourses()) id2c.put(c.id,c);
    if(target!=null){
...
```java
case "grades_export":{
    java.util.List<com.training.db.Db.Grade> list=com.training.db.Db.getGrades(uid);
...
case "logs_query":{
    ...
    int page=jreq.has("page")? jreq.getInt("page"):1;
    int size=jreq.has("size")? jreq.getInt("size"):10;
    java.util.List<String> logs=com.training.db.Db.getLogs();
