...
- URL 入口示例：`POST /api/user`，请求体：`{"action":"login","username":"...","password":"..."}`
- 参数解析说明：业务请求体统一为 JSON，由基础类读取；如需解析查询字符串可用 `lwFormData.parse(req.getQueryString(), charset)`。
- 统一响应：由基础类统一输出 JSON：`{"error":0,"reason":"ok","data":...}`；错误时 `error!=0`，`reason` 为错误原因。前端统一使用 `LW.rest` 以 `POST` JSON 调用。- 补充说明（实际代码实现，初学者必读）：上述 `ok/fail` 为概念化描述，当前项目实际采用“成功直接 return 数据；失败抛出 `lwWebException(code, reason)`”，由 `SimpleRestful` 基类统一格式化为 `{"error":0,"reason":"ok","data":...}` 或 `{"error":code,"reason":message}`。前端判断 `error==0` 即成功：
```
return com.training.dao.DaoFactory.course().listCourses();
// 或（失败时）
...
## 三、核心类与职责（代码走读）
- `Db`（内存数据库，线程安全）：提供实体与操作；Web 层通过 `DaoFactory` 间接访问
...
- `TextFile`：CSV 写文件工具，用于 `grades_export`
...
protected Object execute(HttpServletRequest req, HttpServletResponse resp, org.json.JSONObject jreq) throws Exception {
    ...
    switch(action){
        ...
        case "drop":{
            String courseId = jreq.getString("courseId");
            boolean ok = com.training.dao.DaoFactory.enrollment().drop(uid, courseId);
            return new org.json.JSONObject().put("ok", ok);
        }
        case "mylist":{
            java.util.List<com.training.db.Db.Enrollment> list = com.training.dao.DaoFactory.enrollment().listUserEnrollments(uid);
...
```
- 冲突与容量：通过 `EnrollmentDao.enroll(userId, courseId)` 发起；内部先做时间冲突检查（`conflict(userId,Course)`），未满员则直接 `enrolled`，满员加入候补 `waitlist`。所有操作在课程级锁 `courseLocks` 下保证并发安全，并写入日志。
- 退课与转正：通过 `EnrollmentDao.drop(userId, courseId)` 发起；成功后会减少课程已选人数；若存在候补队列，自动将队首转为已选并记录 `promote` 日志。
- 我的选课：通过 `EnrollmentDao.listUserEnrollments(uid)` 返回当前用户的所有选课记录，接口补充课程名，便于前端直接展示。
- 统计输出：通过 `AdminDao.stats()` 获取 `Map<String,Object>`，包含用户数、课程数、选课总数。

## 四、前端交互与约定（示例页面）
- `index.html`：登录/注册表单，调用 `api/user`（`action=login/register/logout/reset`）
...
- `students.html`：课表事件列表（`calendar`）、成绩、推荐，CSV 导出按钮（触发 `grades_export`）
- `admin.html`：展示 `stats` 与 `logs_query` 的结果

## 五、统一风格与约定
- 接口参数统一使用 `action` 指定具体操作
- JSON 响应统一为 `error/reason/data`，前端判断 `error==0` 即成功
- 日志统一通过 `AdminDao.log(text)` 记录（默认委托至 `Db.log`），便于管理员查询
- 错误码建议：400 参数错误、401 未登录、403 无权限、404 不存在、409 业务冲突、500 内部错误

## 六、循序渐进实现顺序（建议）
1. 启动与种子数据：在 `UserServlet.init()` 中通过 `DaoFactory.admin().seed()` 初始化示例用户/课程/选课/成绩
...
- 接口位置：`src/com/training/dao/*.java`
  ...
  - `AdminDao`：`stats/log/getLogs/seed`
- 默认实现：`src/com/training/dao/memory/*.java`（全部委托 `Db` 静态方法；支持 `AdminDao.seed()` 示例数据初始化）
